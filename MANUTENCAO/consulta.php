...
<body class="consulta" style="background-color: #d8d8d8; ">
    ...
    <div class="cardd" >
      <p>ORDEM DE SERVIÇO EM ANDAMENTO (CONSULTA)</p>
...
<div class="dropdown"><label class="filter-text" for="x">Finalizada? </label>
    <button class="filter-button dropdown-toggle" id="x" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><?php
      ...
      if (!isset($selecione) or $selecione ==NULL){
       header('Location: /FORMS/MANUTENCAO/index.php');
      }else{
        echo $selecione;
      }
      ?>
...
<br>
          ...
          <?php
              ...
              try{
                  ...
                  try{
                      ...
                      $sql = "SELECT max(num_solicitacao) as num_solicitacao FROM form_manut";
                  ...
                  if (!isset($num)){
                      ...
                      $cod_setor = $_SESSION['cod-setor'];
                      $departamento = $_SESSION['setor-painel'];
                      ...
                      if ($departamento == 'MAN'){
                        ...
                        if($selecione == 'Todos'){
                         ...
                         x.hora_solicitacao, x.nome_solicitante,x.setor_solicitante, x.num_equipamento, x.nome_equipamento,
                         ...
                         SELECT num_solicitacao, data_solicitacao,hora_solicitacao, nome_solicitante,setor_solicitante, 
                             ...
                             hora_fechamento,nome_responsavel,empresa, num_equipamento, nome_equipamento,
                                   CASE WHEN data_fechamento IS NULL THEN 'NAO' ELSE 'SIM' END AS finalizada
                           FROM form_manut ORDER BY $selecione
                         ) AS x $where";
                       ...
                       // CONSULTA COM TODAS AS SOLICITAÇÕES
                       /* $sql = "SELECT x.num_solicitacao, to_char(x.data_solicitacao, 'DD/MM/YYYY') as data_solicitacao,
                        x.hora_solicitacao, x.nome_solicitante,x.setor_solicitante, x.num_equipamento, x.nome_equipamento,
                        x.tipo_servico, x.nome_recebimento, x.nome_executante,x.descricao,x.status,x.obs_exec,x.data_fechamento,
                        x.hora_fechamento,x.nome_responsavel,x.empresa, x.finalizada
                        ...
                        // CONSULTA APENAS DO SOLICITANTE = LOGADO
                        $sql = "SELECT x.num_solicitacao, to_char(x.data_solicitacao, 'DD/MM/YYYY') as data_solicitacao,
                        x.hora_solicitacao, x.nome_solicitante,x.setor_solicitante, x.num_equipamento, x.nome_equipamento,
                        x.tipo_servico, x.nome_recebimento, x.nome_executante,x.descricao,x.status,x.obs_exec,x.data_fechamento,
                        x.hora_fechamento,x.nome_responsavel,x.empresa, x.finalizada
                        ...
                        SELECT num_solicitacao, data_solicitacao, hora_solicitacao, nome_solicitante,setor_solicitante, tipo_servico, nome_recebimento, 
                                  nome_executante,descricao,status,obs_exec,data_fechamento,hora_fechamento,nome_responsavel,empresa, num_equipamento, nome_equipamento,
                                  CASE WHEN data_fechamento IS NULL THEN 'NAO' ELSE 'SIM' END AS finalizada
                          FROM form_manut
                          WHERE nome_solicitante = '$func'
                          ORDER BY $selecione
                        ) AS x $where ";
                      }
    ...
    <div class="flex ctr">
      ...
      <a class="sair2" href="/FORMS/MANUTENCAO/index.php">
