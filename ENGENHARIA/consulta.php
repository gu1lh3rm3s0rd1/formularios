...
<div class="dropdown"><label class="filter-text" for="x">Finalizada? </label>
    <button class="filter-button dropdown-toggle" id="x" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><?php
      ...
      if (!isset($selecione) or $selecione ==NULL){
       header('Location: /FORMS/ENGENHARIA/index.php');
      }else{
        echo $selecione;
      }
      ?>
    ...
    <button type="submit" class="just-hover-item filter-button-item" name="selecione" value="Todos">Todos</button><br>
    <!--<button type="submit" class="just-hover-item filter-button-item" name="selecione" value="Data Solicitação (Mais antiga)">Data Solicitação (Mais antiga)</button><br>
    <button type="submit" class="just-hover-item filter-button-item" name="selecione" value="Numero Solicitação (Crescente)">Numero Solicitação (Crescente)</button><br>
    <button type="submit" class="just-hover-item filter-button-item" name="selecione" value="Numero Solicitação (Decrescente)">Numero Solicitação (Decrescente)</button><br>
    <button type="submit" class="just-hover-item filter-button-item" name="selecione" value="Nome Solicitante (A-Z)">Nome Solicitante (A-Z)</button><br>
    <button type="submit" class="just-hover-item filter-button-item" name="selecione" value="Nome Solicitante (Z-A)">Nome Solicitante (Z-A)</button><br>-->
    <button type="submit" class="just-hover-item filter-button-item" name="selecione" value="Sim">Sim</button><br>
    <button type="submit" class="just-hover-item filter-button-item" name="selecione" value="Não">Não</button>
...
<br>
          ...
          <?php
              ...
              try{
                  ...
                  try{
                      ...
                      $sql = "SELECT max(num_solicitacao) as num_solicitacao FROM form_eng";
                  ...
                  if (!isset($num)){
                      ...
                      if ($adm == 0){
                        ...
                        if($selecione == 'Todos'){
                          $where = "";
                          $selecione = '(data_solicitacao, hora_solicitacao) DESC' ;
                         } 
                         else if($selecione == 'Sim'){
                          $where = "WHERE finalizada = 'SIM'";
                          $selecione = "(data_solicitacao, hora_solicitacao) DESC";
                         } 
                         else if($selecione == 'Não'){
                        ...
                        SELECT num_solicitacao, data_solicitacao, hora_solicitacao, nome_solicitante,setor_solicitante, tipo_servico, nome_recebimento, 
                          ...
                          FROM form_eng
                          WHERE nome_recebimento = '$func' OR nome_executante = '$func' OR nome_solicitante = '$func'
                          ORDER BY $selecione
                        ) AS x $where ";
                        ...
                        SELECT num_solicitacao, data_solicitacao,hora_solicitacao, nome_solicitante,setor_solicitante, 
                            ...
                            hora_fechamento,nome_responsavel,empresa,
                                  CASE WHEN data_fechamento IS NULL THEN 'NAO' ELSE 'SIM' END AS finalizada
                          FROM form_eng ORDER BY $selecione
                        ) AS x $where";
                      }
                      ...
                      foreach($myPDO->query($sql)as $row){
                        ...
                        //caixa de mensagem que exibe as informacoes das solicitaçoes disponiveis
                        if ($finalizada == 'SIM'){ 
                            $link2 = "href='visualizar.php?id=".$linked."'";
                            echo '
                        ...
                        if ($finalizada == 'NAO'){ 
                            $link2 = "href='nvisualizar.php?id=".$linked."'";
                            echo '
    ...
    <div class="flex ctr">
      ...
      <a class="sair2" href="/FORMS/ENGENHARIA/index.php">
